...
typedef enum {
    ...
    ERR_QUEUE_FULL     = -5,   // 队列�?
    ERR_QUEUE_EMPTY    = -6,   // 队列�?
    ERR_MODULE_NOT_FOUND = -7, // 模块未找�?
    ERR_MODULE_INIT    = -8,   // 模块初始化失�?
    ERR_CONFIG_LOAD    = -9,   // 配置加载失败
    ERR_MSG_SEND       = -10,  // 消息发送失�?
    ERR_MSG_RECV       = -11,  // 消息接收失败
    ERR_CORO_CREATE    = -12,  // 协程创建失败
    ERR_SANDBOX_INIT   = -13,  // 沙箱初始化失�?
    ERR_MODULE_LOAD    = -14,  // 模块加载失败
    ERR_MODULE_INVALID = -15,  // 模块无效
    ERR_NOT_FOUND      = -16,  // 未找�?
    ERR_ALREADY_EXISTS = -17,  // 已存�?
    ERR_BUSY           = -18,  // �?
    ...
    ERR_HEALTH_CHECK_FAIL = -30, // 健康检查失�?

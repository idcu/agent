...
typedef enum { OUTPUT_FORMAT_TEXT, OUTPUT_FORMAT_CSV, OUTPUT_FORMAT_JSON } OutputFormat;
...
static void bench_stats_destroy(BenchmarkStats *stats) { free(stats->durations); }
...
static void bench_suite_add_benchmark(BenchmarkSuite *suite, const char *name, BenchmarkFunc func,
                                      uint64_t iterations, uint64_t warmup_iterations,
                                      uint64_t runs) {
    if (suite->count >= MAX_BENCHMARKS)
        return;
...
static void bench_suite_print_text(BenchmarkSuite *suite) {
    ...
    printf("---------------------------------------------------------------------------------------"
           "-----------------\n");
    printf("%-20s %12s %12s %12s %12s %12s %12s\n", "Benchmark", "Iterations", "Runs", "Mean(us)",
           "Min(us)", "Max(us)", "Ops/sec");
    printf("---------------------------------------------------------------------------------------"
           "-----------------\n");
    ...
    for (int i = 0; i < suite->count; i++) {
        Benchmark *b = &suite->benchmarks[i];
        double ops_per_sec =
            (b->stats.mean_us > 0) ? (double)b->iterations * 1000000.0 / b->stats.mean_us : 0.0;
        printf("%-20s %12llu %12llu %12.0f %12llu %12llu %12.0f\n", b->name,
               (unsigned long long)b->iterations, (unsigned long long)b->runs, b->stats.mean_us,
               (unsigned long long)b->stats.min_us, (unsigned long long)b->stats.max_us,
               ops_per_sec);
    }
    printf("---------------------------------------------------------------------------------------"
           "-----------------\n");
...
static void bench_suite_print_json(BenchmarkSuite *suite) {
    ...
    for (int i = 0; i < suite->count; i++) {
        ...
        printf("{\"name\":\"%s\",\"iterations\":%llu,\"runs\":%llu,"
               ...
               b->name, (unsigned long long)b->iterations, (unsigned long long)b->runs,
               b->stats.mean_us, (unsigned long long)b->stats.min_us,
               (unsigned long long)b->stats.max_us, b->stats.stddev_us, ops_per_sec);
    }
    printf("]}\n");
}
...
static int bench_suite_load_baseline(const char *filename, BaselineData *baseline) {
    ...
    while (fgets(line, sizeof(line), fp)) {
        if (sscanf(line, "      \"name\": \"%255[^\"]\",", name) == 1) {
            ...
            baseline->count++;
            if (baseline->count >= MAX_BENCHMARKS)
                break;
        }
    }
...
static int bench_suite_check_regression(BenchmarkSuite *suite, BaselineData *baseline,
                                        double threshold) {
    int regressions_found = 0;

    printf("\n=== Performance Regression Check ===\n");
    printf("Threshold: %.1f%%\n\n", threshold);
    printf("%-20s %15s %15s %15s %10s\n", "Benchmark", "Baseline(ops/s)", "Current(ops/s)",
           "Change", "Status");
    printf("--------------------------------------------------------------------------------\n");
    ...
    for (int i = 0; i < suite->count; i++) {
        ...
        if (base) {
            ...
            printf("%-20s %15.0f %15.0f %14.1f%% %10s\n", b->name, base->ops_per_sec, current_ops,
                   change, is_regression ? "REGRESSION" : "OK");
        } else {
            printf("%-20s %15s %15.0f %15s %10s\n", b->name, "N/A", current_ops, "N/A", "NEW");
        }
    }
...
int main(int argc, char *argv[]) {
    ...
    bench_suite_add_benchmark(&suite, "msg_send", bench_msg_send, msg_iterations, warmup_iterations,
                              runs);
    bench_suite_add_benchmark(&suite, "msg_recv", bench_msg_recv, msg_iterations, warmup_iterations,
                              runs);
    bench_suite_add_benchmark(&suite, "msg_roundtrip", bench_msg_roundtrip, msg_iterations,
                              warmup_iterations, runs);
    bench_suite_add_benchmark(&suite, "msg_batch_send", bench_msg_batch_send, msg_iterations,
                              warmup_iterations, runs);
    bench_suite_add_benchmark(&suite, "memory_pool_alloc", bench_memory_pool_alloc, mem_iterations,
                              warmup_iterations, runs);
    bench_suite_add_benchmark(&suite, "memory_pool_alloc_free", bench_memory_pool_alloc_free,
                              mem_iterations, warmup_iterations, runs);
    bench_suite_add_benchmark(&suite, "coro_create_destroy", bench_coro_create, coro_iterations,
                              warmup_iterations, runs);
    bench_suite_add_benchmark(&suite, "coro_switch", bench_coro_switch, coro_iterations * 10,
                              warmup_iterations, runs);
    bench_suite_add_benchmark(&suite, "coro_schedule", bench_coro_schedule, coro_iterations,
                              warmup_iterations, runs);
    ...
    if (g_check_regression && g_baseline_file) {
        ...
        if (bench_suite_load_baseline(g_baseline_file, &baseline) == 0) {
            int regressions =
                bench_suite_check_regression(&suite, &baseline, g_regression_threshold);
            if (regressions > 0) {
                ret = 1;
            }
        } else {

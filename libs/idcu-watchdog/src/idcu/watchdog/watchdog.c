...
int idcu_watchdog_init(idcu_Watchdog *wd, uint32_t timeout_ms, uint32_t max_restarts,
    ...
    wd->restart_window_ms =
        restart_window_ms > 0 ? restart_window_ms : IDCU_WATCHDOG_RESTART_WINDOW_MS;
    wd->last_heartbeat = get_current_time_ms();
...
int idcu_watchdog_set_on_timeout(idcu_Watchdog *wd, idcu_WatchdogCallback callback,
                                 void *user_data) {
    (void)wd;
...
int idcu_watchdog_set_on_restart(idcu_Watchdog *wd, idcu_WatchdogCallback callback,
                                 void *user_data) {
    (void)wd;

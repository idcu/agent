...
int idcu_module_security_context_create(idcu_ModuleSecurityContext **ctx) {
    ...
    idcu_ModuleSecurityContext *context =
        (idcu_ModuleSecurityContext *)malloc(sizeof(idcu_ModuleSecurityContext));
    if (!context) {
...
int idcu_module_security_add_trusted_key(idcu_ModuleSecurityContext *ctx,
                                         const idcu_SigningKey *key) {
    if (!ctx || !key) {
        return IDCU_ERROR_INVALID_ARG;
    }
    if (!ctx->initialized) {
...
int idcu_module_signature_verify(idcu_ModuleSecurityContext *ctx, const void *module_data,
                                 size_t module_size, const idcu_ModuleSignature *signature) {
    ...
    idcu_log_info("Module signature verified successfully for %s v%s", signature->module_name,
...
int idcu_module_signature_sign(const void *module_data, size_t module_size,
                               ...
                               idcu_SignatureAlgorithm alg, idcu_ModuleSignature *signature) {
...
int idcu_module_signature_serialize(const idcu_ModuleSignature *signature, char *buffer,
    ...
    idcu_security_hex_encode(signature->module_hash, IDCU_MODULE_HASH_SIZE, hex_hash,
                             sizeof(hex_hash));
    idcu_security_hex_encode(signature->signature, signature->signature_size, hex_sig,
                             sizeof(hex_sig));
...
int idcu_config_encrypt(const void *plaintext, size_t plaintext_size, const uint8_t *key,
                        size_t key_size, idcu_EncryptedConfig *encrypted) {
    ...
    ret = idcu_security_encrypt(IDCU_SECURITY_ALG_AES_256_CBC, key, key_size, encrypted->iv,
                                encrypted->iv_size, plaintext, plaintext_size, encrypted->data,
                                &ciphertext_size);
    if (ret != IDCU_OK) {
        return ret;
    }
...
int idcu_config_decrypt(const idcu_EncryptedConfig *encrypted, const uint8_t *key, size_t key_size,
    ...
    int ret = idcu_security_decrypt(IDCU_SECURITY_ALG_AES_256_CBC, key, key_size, encrypted->iv,
                                    encrypted->iv_size, encrypted->data, encrypted->data_size,
...
int idcu_encrypted_config_serialize(const idcu_EncryptedConfig *cfg, char *buffer,
                                    size_t buffer_size, size_t *output_size) {

...
#endif
{
    ...
    idcu_log_info("Deadlock detector thread started");
    ...
    idcu_log_info("Deadlock detector thread stopped");
...
int idcu_deadlock_detector_init(idcu_DeadlockDetector* detector)
{
    ...
    idcu_log_info("Deadlock detector initialized");
...
int idcu_deadlock_detector_start(idcu_DeadlockDetector* detector)
{
    ...
    if (!detector->thread_handle) {
        idcu_log_error("Failed to create deadlock detector thread");
        detector->enabled = 0;
        return IDCU_ERR_SYSTEM;
    }
#else
    if (pthread_create((pthread_t*)&detector->thread_handle, NULL, deadlock_detector_thread, detector) != 0) {
        idcu_log_error("Failed to create deadlock detector thread");
        detector->enabled = 0;
        return IDCU_ERR_SYSTEM;
    }
    ...
    idcu_log_info("Deadlock detector started");
    return IDCU_ERR_OK;
}
...
int idcu_deadlock_detector_stop(idcu_DeadlockDetector* detector)
{
    ...
    idcu_log_info("Deadlock detector stopped");
    return IDCU_ERR_OK;
}
...
int idcu_deadlock_detector_register_lock(idcu_DeadlockDetector* detector, idcu_Mutex* lock, const char* name)
{
    ...
    idcu_mutex_unlock(&detector->detector_lock);
    idcu_log_error("No available slot to register lock");
...
int idcu_deadlock_detector_register_thread(idcu_DeadlockDetector* detector, const char* name)
{
    ...
    idcu_mutex_unlock(&detector->detector_lock);
    idcu_log_error("No available slot to register thread");
    return IDCU_ERR_NO_MEMORY;
}
...
void idcu_deadlock_detector_check(idcu_DeadlockDetector* detector)
{
    ...
    for (int i = 0; i < IDCU_DEADLOCK_MAX_THREADS; i++) {
        ...
        if (wait_time > IDCU_DEADLOCK_MAX_WAIT_MS) {
            ...
            const char* thread1_name = detector->threads[i].thread_name ? detector->threads[i].thread_name : "unknown";
            
            for (int j = 0; j < IDCU_DEADLOCK_MAX_LOCKS; j++) {
                if (detector->locks[j].in_use && detector->locks[j].lock == waiting_for) {
                    const char* lock1_name = detector->locks[j].lock_name ? detector->locks[j].lock_name : "unknown";
                    
                    for (int k = 0; k < IDCU_DEADLOCK_MAX_THREADS; k++) {
                        if (k == i) continue;
                        
                        if (thread_ids_equal(detector->threads[k].thread_id, detector->locks[j].owner_thread) && 
                            detector->threads[k].is_waiting) {
                            
                            for (int l = 0; l < IDCU_DEADLOCK_MAX_LOCKS; l++) {
                                if (detector->locks[l].in_use && detector->locks[l].lock == detector->threads[k].waiting_for) {
                                    if (thread_ids_equal(detector->locks[l].owner_thread, detector->threads[i].thread_id)) {
                                        const char* thread2_name = detector->threads[k].thread_name ? detector->threads[k].thread_name : "unknown";
                                        const char* lock2_name = detector->locks[l].lock_name ? detector->locks[l].lock_name : "unknown";
                                        
                                        idcu_log_error("Potential deadlock detected: Thread '%s' waiting for lock '%s', "

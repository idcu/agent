...
static idcu_ErrorInfo g_last_error = {0};  // NOLINT(cppcoreguidelines-avoid-non-const-global-variables)
...
const char *idcu_err_to_str(int err_code) {
    ...
    case IDCU_ERR_LIMIT_EXCEEDED:
        return "Limit exceeded";
    case IDCU_ERR_IO_ERROR:
        return "IO error";
    case IDCU_ERR_SECURITY_ERROR:
        return "Security error";
    case IDCU_ERR_RESOURCE_EXHAUSTED:
        return "Resource exhausted";
    case IDCU_ERR_VERIFICATION_FAILED:
...
void idcu_err_set_last_error(int err_code, const char *context, const char *file, int line) {  // NOLINT(bugprone-easily-swappable-parameters)
    g_last_error.error_code = err_code;
    g_last_error.file = file;
    g_last_error.line = line;

    if (context) {
        strncpy(g_last_error.context, context, IDCU_ERROR_CONTEXT_MAX_LEN - 1);  // NOLINT(clang-analyzer-security.insecureAPI.DeprecatedOrUnsafeBufferHandling)
...
const idcu_ErrorInfo *idcu_err_get_last_error(void) { return &g_last_error; }

void idcu_err_clear_last_error(void) { memset(&g_last_error, 0, sizeof(g_last_error)); }  // NOLINT(clang-analyzer-security.insecureAPI.DeprecatedOrUnsafeBufferHandling)

...
typedef enum
{
    IDCU_DEADLOCK_LOG_LEVEL_DEBUG = 0,
    IDCU_DEADLOCK_LOG_LEVEL_INFO,
    IDCU_DEADLOCK_LOG_LEVEL_WARNING,
    IDCU_DEADLOCK_LOG_LEVEL_ERROR
} idcu_DeadlockLogLevel;

typedef void (*idcu_DeadlockCallback)(const char* thread1, const char* thread2, const char* lock1,
                                      const char* lock2, void* user_data);

typedef void (*idcu_DeadlockLogCallback)(idcu_DeadlockLogLevel level, const char* message,
                                         void* user_data);
...
typedef struct
{
    ...
    void*                    callback_data;
    idcu_DeadlockLogCallback log_callback;

...
int idcu_server_monitor_get_system_info(idcu_ServerMonitor *monitor, idcu_SystemInfo *info) {
    ...
    if (GetVersionEx((OSVERSIONINFO *)&osvi)) {
        snprintf(info->os_version, sizeof(info->os_version), "%lu.%lu.%lu", osvi.dwMajorVersion,
                 osvi.dwMinorVersion, osvi.dwBuildNumber);
    }

    ULONGLONG uptime = GetTickCount64() / 1000;
    info->uptime_seconds = (uint64_t)uptime;
    snprintf(info->uptime, sizeof(info->uptime), "%llu days, %llu hours, %llu minutes",
    ...
    if (mounts) {
        ...
        while (fgets(line, sizeof(line), mounts) && *count < max_count) {
            ...
            if (sscanf(line, "%127s %255s %63s", device, mount, fs) == 3) {
                if (strcmp(fs, "ext4") == 0 || strcmp(fs, "xfs") == 0 || strcmp(fs, "btrfs") == 0 ||
                    strcmp(fs, "vfat") == 0) {
                    strncpy(disks[*count].device, device, sizeof(disks[*count].device) - 1);
                    strncpy(disks[*count].mount_point, mount,
                    ...
                    if (statvfs(mount, &stat) == 0) {
                        ...
                        disks[*count].used_bytes =
                            disks[*count].total_bytes - disks[*count].free_bytes;
                    }
                    (*count)++;
                }
            }
...
int idcu_server_monitor_get_network_info(idcu_ServerMonitor *monitor, idcu_NetworkInfo *networks,
                                         int *count) {
    if (!monitor || !networks || !count)
        return IDCU_ERR_INVALID_PARAM;
...
int idcu_server_monitor_export_json(idcu_ServerMonitor *monitor, char *buffer, size_t buffer_size,
    ...
    int written =
        snprintf(buffer, buffer_size,
                 ...
                 "},"
                 "\"disks\":[",
                 (unsigned long long)metrics.timestamp_ms, metrics.system.hostname,
                 metrics.system.os_name, metrics.system.os_version, metrics.system.kernel_version,
                 metrics.system.uptime, (unsigned long long)metrics.system.uptime_seconds,
                 metrics.cpu.load_1min, metrics.cpu.load_5min, metrics.cpu.load_15min,
                 (unsigned long long)metrics.memory.total_bytes,
                 (unsigned long long)metrics.memory.used_bytes,
                 (unsigned long long)metrics.memory.free_bytes, metrics.memory.usage_percent);
    ...
    for (int i = 0; i < metrics.disk_count; i++) {
        written +=
            snprintf(buffer + written, buffer_size - written,
                     ...
                     metrics.disks[i].device, metrics.disks[i].mount_point,
                     (unsigned long long)metrics.disks[i].total_bytes, metrics.disks[i].device,
                     metrics.disks[i].mount_point, (unsigned long long)metrics.disks[i].used_bytes,
                     metrics.disks[i].device, metrics.disks[i].mount_point,
                     (unsigned long long)metrics.disks[i].free_bytes);
    }

...
static int handle_status(idcu_HttpRequest* request, idcu_HttpResponse* response, void* user_data) {
    ...
    snprintf(json, sizeof(json), 
             ...
             status.version, (unsigned long long)status.uptime_seconds, 
             status.uptime_str, status.running ? "true" : "false");
...
static int handle_modules(idcu_HttpRequest* request, idcu_HttpResponse* response, void* user_data) {
    ...
    for (size_t i = 0; i < actual_count; i++) {
        ...
        offset += snprintf(json + offset, sizeof(json) - offset, 
                         ...
                         modules[i].info.author, modules[i].info.description,
                         modules[i].info.priority,
                         modules[i].loaded ? "true" : "false",
                         modules[i].running ? "true" : "false");
    }
...
static int handle_module_load(idcu_HttpRequest* request, idcu_HttpResponse* response, void* user_data) {
    (void)user_data;
...
static int handle_module_unload(idcu_HttpRequest* request, idcu_HttpResponse* response, void* user_data) {
    (void)user_data;
...
static int handle_healthcheck(idcu_HttpRequest* request, idcu_HttpResponse* response, void* user_data) {
    (void)request;
    (void)user_data;
...
int idcu_http_management_init(idcu_HttpManagementServer* server, const char* address, uint16_t port) {
    ...
    idcu_http_server_register_route(&server->server, IDCU_HTTP_METHOD_GET, "/api/status", handle_status, NULL);
    idcu_http_server_register_route(&server->server, IDCU_HTTP_METHOD_GET, "/api/modules", handle_modules, NULL);
    idcu_http_server_register_route(&server->server, IDCU_HTTP_METHOD_POST, "/api/modules/load", handle_module_load, NULL);
    idcu_http_server_register_route(&server->server, IDCU_HTTP_METHOD_POST, "/api/modules/unload", handle_module_unload, NULL);
    idcu_http_server_register_route(&server->server, IDCU_HTTP_METHOD_POST, "/api/healthcheck", handle_healthcheck, NULL);
    idcu_http_server_register_route(&server->server, IDCU_HTTP_METHOD_GET, "/api/metrics", handle_metrics, NULL);
    idcu_http_server_register_route(&server->server, IDCU_HTTP_METHOD_GET, "/api/config", handle_config, NULL);

...
static int parse_array(idcu_JsonParser *p, idcu_JsonValue *out) {
    ...
    while (p->pos < p->end && *p->pos != ']') {
        ...
        if (array->count >= array->capacity) {
            ...
            idcu_JsonValue *new_elems =
                (idcu_JsonValue *)realloc(array->elements, new_cap * sizeof(idcu_JsonValue));
            if (!new_elems) {
...
static int parse_object(idcu_JsonParser *p, idcu_JsonValue *out) {
    ...
    while (p->pos < p->end && *p->pos != '}') {
        ...
        if (obj->count >= obj->capacity) {
            ...
            idcu_JsonValue *new_vals =
                (idcu_JsonValue *)realloc(obj->values, new_cap * sizeof(idcu_JsonValue));
...
static int json_to_string_internal(const idcu_JsonValue *value, char *buffer, size_t *offset,
                                   size_t buffer_size) {
    ...
    case IDCU_JSON_TYPE_DOUBLE:
        len = snprintf(buffer + *offset, buffer_size - *offset, "%f", value->data.double_val);
        break;
    case IDCU_JSON_TYPE_STRING:
        len = snprintf(buffer + *offset, buffer_size - *offset, "\"%s\"", value->data.string_val);
        break;
    case IDCU_JSON_TYPE_ARRAY:
        len = snprintf(buffer + *offset, buffer_size - *offset, "[");
        if (len > 0)
        ...
        for (size_t i = 0; i < value->data.array_val->count; i++) {
            ...
            int ret = json_to_string_internal(&value->data.array_val->elements[i], buffer, offset,
                                              buffer_size);
            if (ret != IDCU_ERR_SUCCESS)
                return ret;
        }
    ...
    case IDCU_JSON_TYPE_OBJECT:
        ...
        for (size_t i = 0; i < value->data.object_val->count; i++) {
            ...
            len = snprintf(buffer + *offset, buffer_size - *offset,
                           "\"%s\":", value->data.object_val->keys[i]);
            if (len > 0)
                *offset += len;
            int ret = json_to_string_internal(&value->data.object_val->values[i], buffer, offset,
                                              buffer_size);
            if (ret != IDCU_ERR_SUCCESS)
                return ret;
        }

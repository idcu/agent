...
static const char *g_status_texts[] = {[200] = "OK",
                                       [201] = "Created",
                                       [400] = "Bad Request",
                                       [404] = "Not Found",
...
idcu_HttpMethod idcu_http_method_from_string(const char *method_str) {
    if (strcmp(method_str, "GET") == 0)
        return IDCU_HTTP_METHOD_GET;
    if (strcmp(method_str, "POST") == 0)
        return IDCU_HTTP_METHOD_POST;
    if (strcmp(method_str, "PUT") == 0)
        return IDCU_HTTP_METHOD_PUT;
    if (strcmp(method_str, "DELETE") == 0)
        return IDCU_HTTP_METHOD_DELETE;
    return IDCU_HTTP_METHOD_UNKNOWN;
}

const char *idcu_http_status_text(int status_code) {
    if (status_code >= 0 &&
        status_code < (int)(sizeof(g_status_texts) / sizeof(g_status_texts[0])) &&
        g_status_texts[status_code]) {
...
int idcu_http_response_add_header(idcu_HttpResponse *response, const char *name,
                                  const char *value) {
    if (!response || !name || !value)
        return IDCU_ERR_INVALID_PARAM;
    if (response->header_count >= IDCU_HTTP_MAX_HEADER)
...
static int send_http_response(idcu_NetworkSocket *sock, idcu_HttpResponse *response) {
    ...
    int offset = snprintf(header_buf, sizeof(header_buf), "HTTP/1.1 %d %s\r\n",
                          response->status_code, response->status_text);

    for (int i = 0; i < response->header_count; i++) {
        offset += snprintf(header_buf + offset, sizeof(header_buf) - offset, "%s: %s\r\n",
...
int idcu_http_server_init(idcu_HttpServer *server, const char *address, uint16_t port) {
    if (!server)
        return IDCU_ERR_INVALID_PARAM;
...
int idcu_http_server_register_route(idcu_HttpServer *server, idcu_HttpMethod method,
    ...
    if (!server || !path || !handler)
        return IDCU_ERR_INVALID_PARAM;
    if (server->route_count >= IDCU_HTTP_MAX_ROUTES)
        return IDCU_ERR_QUEUE_FULL;
    ...
    IDCU_LOG_DEBUG("Registered route: %s %s",
                   method == IDCU_HTTP_METHOD_GET      ? "GET"
                   : method == IDCU_HTTP_METHOD_POST   ? "POST"
                   : method == IDCU_HTTP_METHOD_PUT    ? "PUT"
                   : method == IDCU_HTTP_METHOD_DELETE ? "DELETE"
                                                       : "UNKNOWN",
                   path);
    return IDCU_ERR_OK;
}
                ...
                if (server->routes[i].method == request.method) {
                    ...
                    if (route_len > 0 && route_path[route_len - 1] == '*') {
                        ...
                        if (strncmp(route_path, request.path, prefix_len) == 0) {
                            ret = server->routes[i].handler(&request, &response,
                                                            server->routes[i].user_data);
                            handled = 1;
                            break;
                        }
                    }

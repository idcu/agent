...
void idcu_metrics_destroy(idcu_MetricsCollector *collector) {
    ...
    IDCU_VECTOR_FOR_EACH(&collector->metrics_list, idcu_Metric *, metric, i) { free(*metric); }
...
int idcu_metrics_register_with_labels(idcu_MetricsCollector *collector, const char *name,
    ...
    if (labels && label_count > 0) {
        int copy_count =
            label_count < IDCU_METRIC_MAX_LABELS ? label_count : IDCU_METRIC_MAX_LABELS;
...
int idcu_metrics_set_histogram_buckets(idcu_MetricsCollector *collector, const char *name,
    ...
    int copy_count =
        bucket_count < IDCU_HISTOGRAM_MAX_BUCKETS ? bucket_count : IDCU_HISTOGRAM_MAX_BUCKETS;
    for (int i = 0; i < copy_count; i++) {
    ...
    IDCU_VECTOR_FOR_EACH(&collector->metrics_list, idcu_Metric *, metric_ptr, i) {
        ...
        case IDCU_METRIC_COUNTER:
            idcu_strbuf_append_format(buf, "%s: %llu (counter) - %s\n", m->name,
                                      (unsigned long long)m->value, m->desc);
            break;
        ...
        case IDCU_METRIC_SUMMARY:
            if (m->count > 0) {
                idcu_strbuf_append_format(
                    buf, "%s: count=%llu sum=%llu min=%llu max=%llu (histogram/summary) - %s\n",
                    m->name, (unsigned long long)m->count, (unsigned long long)m->sum,
                    (unsigned long long)m->min, (unsigned long long)m->max, m->desc);
            } else {
        ...
        case IDCU_METRIC_HISTOGRAM:
            ...
            if (m->bucket_count > 0) {
                for (int b = 0; b < m->bucket_count; b++) {
                    if (collector->namespace_prefix[0] != '\0') {
                        idcu_strbuf_append_format(buf, "%s_%s_bucket", collector->namespace_prefix,
                                                  m->name);
                    } else {
                    ...
                    if (m->label_count > 0) {
                        ...
                        for (int l = 0; l < m->label_count; l++) {
                            if (l > 0)
                                idcu_strbuf_append(buf, ",");
                            idcu_strbuf_append_format(buf, "%s=\"%s\"", m->labels[l].key,
                                                      m->labels[l].value);
                        }
...
const char *idcu_metrics_get_grafana_dashboard_json(void) { return g_grafana_dashboard_json; }
...
int idcu_global_metrics_register_with_labels(const char *name, const char *desc,
                                             idcu_MetricType type, const idcu_MetricLabel *labels,
                                             int label_count) {
    if (!g_global_initialized) {
        return IDCU_ERR_NOT_INITIALIZED;
    }
    return idcu_metrics_register_with_labels(&g_global_collector, name, desc, type, labels,
                                             label_count);
}

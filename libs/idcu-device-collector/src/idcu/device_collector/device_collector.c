...
void idcu_device_collector_destroy(idcu_DeviceCollector *collector) {
    if (!collector || !collector->internal)
        return;
    free(collector->internal);
    collector->internal = NULL;
}

static int find_device_index(struct idcu_DeviceCollectorInternal *internal,
...
int idcu_device_collector_register_device(idcu_DeviceCollector *collector,
    ...
    if (!collector || !collector->internal || !device)
        return IDCU_ERR_INVALID_PARAM;
    ...
    if (internal->status_callback) {
        internal->status_callback(device->name, device->status,
                                  internal->status_callback_user_data);
    }
...
int idcu_device_collector_unregister_device(idcu_DeviceCollector *collector,
                                            const char *device_name) {
    if (!collector || !collector->internal || !device_name)
        return IDCU_ERR_INVALID_PARAM;
...
int idcu_device_collector_get_device(idcu_DeviceCollector *collector, const char *device_name,
    ...
    if (!collector || !collector->internal || !device_name || !device)
        return IDCU_ERR_INVALID_PARAM;
...
int idcu_device_collector_list_devices(idcu_DeviceCollector *collector, idcu_DeviceInfo *devices,
                                       int *count) {
    if (!collector || !collector->internal || !devices || !count)
        return IDCU_ERR_INVALID_PARAM;
...
int idcu_device_collector_update_device_status(idcu_DeviceCollector *collector,
                                               const char *device_name, idcu_DeviceStatus status) {
    if (!collector || !collector->internal || !device_name)
        return IDCU_ERR_INVALID_PARAM;
...
int idcu_device_collector_add_data_point(idcu_DeviceCollector *collector, const char *device_name,
                                         const idcu_DeviceDataPoint *data) {
    if (!collector || !collector->internal || !device_name || !data)
        return IDCU_ERR_INVALID_PARAM;
...
int idcu_device_collector_set_status_callback(idcu_DeviceCollector *collector,
                                              idcu_DeviceStatusCallback callback, void *user_data) {
    if (!collector || !collector->internal)
        return IDCU_ERR_INVALID_PARAM;
...
static void append_data_point_json(char *buffer, size_t *offset, size_t buffer_size,
                                   const idcu_DeviceDataPoint *data) {
    const char *type_str = "";
    switch (data->type) {
    ...
    int written = snprintf(buffer + *offset, buffer_size - *offset,
                           "{\"key\":\"%s\",\"type\":\"%s\",", data->key, type_str);
    ...
    switch (data->type) {
    case IDCU_DATA_TYPE_INT:
        written = snprintf(buffer + *offset, buffer_size - *offset, "\"value\":%lld,",
                           (long long)data->value.int_val);
        break;
    case IDCU_DATA_TYPE_FLOAT:
        written = snprintf(buffer + *offset, buffer_size - *offset, "\"value\":%.2f,",
                           data->value.float_val);
        break;
    case IDCU_DATA_TYPE_STRING:
        written = snprintf(buffer + *offset, buffer_size - *offset, "\"value\":\"%s\",",
                           data->value.str_val);
        break;
    case IDCU_DATA_TYPE_BOOL:
        written = snprintf(buffer + *offset, buffer_size - *offset, "\"value\":%s,",
    ...
    written = snprintf(buffer + *offset, buffer_size - *offset, "\"timestamp\":%llu}",
...
static void append_device_json(char *buffer, size_t *offset, size_t buffer_size,
                               const idcu_DeviceInfo *device) {
...
int idcu_device_collector_export_json(idcu_DeviceCollector *collector, char *buffer,
    ...
    if (!collector || !collector->internal || !buffer || !output_size)
        return IDCU_ERR_INVALID_PARAM;
...
int idcu_device_collector_export_device_json(idcu_DeviceCollector *collector,
                                             ...
                                             size_t buffer_size, size_t *output_size) {
    if (!collector || !collector->internal || !device_name || !buffer || !output_size)
        return IDCU_ERR_INVALID_PARAM;

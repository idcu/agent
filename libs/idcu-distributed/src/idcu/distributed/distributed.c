...
static int serialize_message(const idcu_NodeMessage *msg, uint8_t *buffer, size_t buffer_size,
...
static void update_node_status_counters(idcu_DistributedNode *node) {
    ...
    for (int i = 0; i < node->node_count; i++) {
        if (node->nodes[i].node_id == node->self_node_id)
            continue;
...
int idcu_distributed_node_add_node(idcu_DistributedNode *node, uint64_t node_id, const char *name,
    ...
    if (!node || !name || !address)
        return IDCU_ERR_INVALID_PARAM;
    if (node->node_count >= IDCU_MAX_NODES)
        return IDCU_ERR_NO_MEMORY;
    if (idcu_distributed_node_find_node(node, node_id))
...
uint64_t idcu_distributed_node_select_node_by_load(idcu_DistributedNode *node) {
    ...
    for (int i = 0; i < node->node_count; i++) {
        if (node->nodes[i].status == IDCU_NODE_STATUS_ONLINE &&
            node->nodes[i].node_id != node->self_node_id) {
...
uint64_t idcu_distributed_node_select_node(idcu_DistributedNode *node) {
    if (!node || node->node_count <= 1)
        return node->self_node_id;
...
static int send_internal_message(idcu_DistributedNode *node, idcu_NodeMessage *msg) {
    ...
    if (ret != IDCU_ERR_OK) {
        IDCU_LOG_ERROR("Failed to send message to node %llu, falling back to local queue",
                       (unsigned long long)msg->to_node);
...
int idcu_distributed_node_send_message(idcu_DistributedNode *node, uint64_t to_node, uint32_t type,
    ...
    if (!node)
        return IDCU_ERR_INVALID_PARAM;
    if (payload && payload_size > IDCU_MAX_MESSAGE_PAYLOAD_SIZE)
        return IDCU_ERR_INVALID_PARAM;
...
int idcu_distributed_node_send_message_reliable(idcu_DistributedNode *node, uint64_t to_node,
    ...
    if (payload && payload_size > IDCU_MAX_MESSAGE_PAYLOAD_SIZE)
        return IDCU_ERR_INVALID_PARAM;
    if (node->pending_msg_count >= IDCU_MAX_PENDING_MESSAGES)
        return IDCU_ERR_QUEUE_FULL;
...
int idcu_distributed_node_start_server(idcu_DistributedNode *node) {
    ...
    int ret =
        idcu_network_server_create(&node->server, IDCU_NET_PROTO_TCP, self->address, self->port);
    if (ret != IDCU_ERR_OK) {
...
int idcu_distributed_node_connect_to_node(idcu_DistributedNode *node, uint64_t node_id) {
    ...
    if (ret != IDCU_ERR_OK) {
        IDCU_LOG_ERROR("Failed to connect to node %llu at %s:%u", (unsigned long long)node_id,
...
int idcu_distributed_node_reconnect_to_node(idcu_DistributedNode *node, uint64_t node_id) {
    ...
    IDCU_LOG_INFO("Attempting to reconnect to node %llu (attempt %u/%u)",
                  (unsigned long long)node_id, (unsigned int)target->reconnect_attempts + 1,
    ...
    for (int i = 0; i < node->pending_msg_count; i++) {
        ...
        if (current_time - node->pending_msgs[i].sent_time > 5000) {
            if (node->pending_msgs[i].retries < 3) {
                ...
                IDCU_LOG_DEBUG("Retransmitting message seq=%u",
                               node->pending_msgs[i].msg.sequence_num);
            } else {
                node->pending_msgs[i].pending = 0;
                IDCU_LOG_ERROR("Message delivery failed after retries, seq=%u",
                               node->pending_msgs[i].msg.sequence_num);
            }
        }
    }
...
static void check_node_timeouts(idcu_DistributedNode *node) {
    ...
    for (int i = 0; i < node->node_count; i++) {
        ...
        if (node_info->node_id == node->self_node_id)
            continue;

        if (node_info->status == IDCU_NODE_STATUS_ONLINE &&
            current_time - node_info->last_heartbeat > IDCU_HEARTBEAT_TIMEOUT_MS) {
            IDCU_LOG_WARN("Node %llu heartbeat timeout, marking as offline",
                          (unsigned long long)node_info->node_id);
            node_info->status = IDCU_NODE_STATUS_OFFLINE;
            node_info->next_reconnect_time = current_time;
            update_node_status_counters(node);
        }

        if ((node_info->status == IDCU_NODE_STATUS_OFFLINE ||
             node_info->status == IDCU_NODE_STATUS_RECONNECTING) &&
...
static void send_heartbeats(idcu_DistributedNode *node) {
    ...
    if (current_time - last_heartbeat_send < IDCU_HEARTBEAT_INTERVAL_MS)
        return;
    ...
    for (int i = 0; i < node->node_count; i++) {
        ...
        idcu_distributed_node_send_message(node, node_info->node_id, IDCU_MSG_TYPE_HEARTBEAT, NULL,
                                           0);
    }
}
...
int idcu_distributed_node_process(idcu_DistributedNode *node) {
            ...
            if (ret == IDCU_ERR_OK && received > 0) {
                ...
                if (ret == IDCU_ERR_OK) {
                    if (msg.to_node == node->self_node_id) {
                        ...
                        idcu_NodeInfo *sender =
                            idcu_distributed_node_find_node(node, msg.from_node);
                        if (sender) {
    ...
    for (int i = 0; i < node->node_count; i++) {
        if (node->nodes[i].connected && node->nodes[i].node_id != node->self_node_id) {
            ...
            size_t received;
            int ret =
                idcu_network_socket_recv(&node->nodes[i].socket, buffer, sizeof(buffer), &received);
            if (ret == IDCU_ERR_OK && received > 0) {
                idcu_NodeMessage msg;
                ret = deserialize_message(buffer, received, &msg);
                if (ret == IDCU_ERR_OK) {
...
int idcu_distributed_node_get_metrics(idcu_DistributedNode *node,
                                      idcu_DistributedMetrics *metrics) {
    if (!node || !metrics)
        return IDCU_ERR_INVALID_PARAM;
    ...
    for (int i = 0; i < node->node_count; i++) {
        if (node->nodes[i].node_id != node->self_node_id &&
            node->nodes[i].status == IDCU_NODE_STATUS_ONLINE) {
...
int idcu_distributed_node_get_node_metrics(idcu_DistributedNode *node, uint64_t node_id,
                                           idcu_NodeMetrics *node_metrics) {
    if (!node || !node_metrics)
        return IDCU_ERR_INVALID_PARAM;

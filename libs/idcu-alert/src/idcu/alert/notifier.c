...
static const char *get_state_str(idcu_AlertState state) {
    switch (state) {
    case IDCU_ALERT_STATE_INACTIVE:
        return "INACTIVE";
    case IDCU_ALERT_STATE_PENDING:
        return "PENDING";
    case IDCU_ALERT_STATE_FIRING:
        return "FIRING";
    case IDCU_ALERT_STATE_RESOLVED:
        return "RESOLVED";
    case IDCU_ALERT_STATE_SILENCED:
        return "SILENCED";
    default:
        return "UNKNOWN";
    }
...
const char *idcu_alert_level_to_string(idcu_AlertLevel level) { return get_level_str(level); }

const char *idcu_alert_state_to_string(idcu_AlertState state) { return get_state_str(state); }
...
static void format_alert_message(char *buf, size_t buf_size, const idcu_AlertEvent *event) {
    ...
    snprintf(buf, buf_size,
             ...
             "State: %s\n"
             "Message: %s\n",
             time_str, event->alert_name, get_level_str(event->level), get_state_str(event->state),
             event->message);
}
...
int idcu_notifier_add_dingtalk_channel(idcu_Notifier *notifier, const char *webhook_url,
    ...
    ch->enabled = 1;
    strncpy(ch->config.dingtalk.webhook_url, webhook_url,
            sizeof(ch->config.dingtalk.webhook_url) - 1);
    if (secret) {
...
int idcu_notifier_set_channel_enabled(idcu_Notifier *notifier, int index, int enabled) {
    ...
    IDCU_LOG_INFO("notifier: channel %d (%s) %s", index,

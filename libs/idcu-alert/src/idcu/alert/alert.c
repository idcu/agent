...
static uint64_t get_current_time_ms_alrt(void) {
    ...
    return (uint64_t)(li.QuadPart / 10000);
#else
    struct timeval tv;
...
static void add_event(idcu_AlertManager *mgr, const idcu_AlertEvent *event) {
    if (mgr->event_count >= IDCU_ALERT_MAX_RULES * 2) {
        memmove(&mgr->events[0], &mgr->events[1], (mgr->event_count - 1) * sizeof(idcu_AlertEvent));
        mgr->event_count--;
    }
    ...
    IDCU_LOG_INFO("alert event: %s [%s] %s", event->alert_name,
                  event->state == IDCU_ALERT_STATE_FIRING ? "FIRING" : "RESOLVED", event->message);
}
...
int idcu_alert_manager_set_callback(idcu_AlertManager *mgr, idcu_AlertCallback cb,
                                    void *user_data) {
    if (!mgr || !mgr->initialized) {
        return IDCU_ERR_INVALID_PARAM;
    }
...
int idcu_alert_manager_get_events(idcu_AlertManager *mgr, idcu_AlertEvent *events, int max_events,
                                  int *count) {
...
void idcu_alert_rule_init(idcu_AlertRule *rule, const char *name, idcu_AlertLevel level,
    ...
    rule->label_count = 0;
    snprintf(rule->message, sizeof(rule->message), "Alert %s triggered for metric %s", name,
             metric_name);
}
...
int idcu_alert_rule_add_label(idcu_AlertRule *rule, const char *key, const char *value) {
    ...
    strncpy(rule->labels[rule->label_count].key, key,
            sizeof(rule->labels[rule->label_count].key) - 1);
    strncpy(rule->labels[rule->label_count].value, value,
            sizeof(rule->labels[rule->label_count].value) - 1);
    rule->label_count++;
    return IDCU_ERR_SUCCESS;
}

...
#define IDCU_TEST_ASSERT_EQ(expected, actual) idcu_test_assert_int_eq((expected), (actual), #expected " != " #actual " at " __FILE__ ":" __LINE__)
#define IDCU_TEST_ASSERT_STR_EQ(expected, actual) idcu_test_assert_str_eq((expected), (actual), "Strings not equal at " __FILE__ ":" __LINE__)

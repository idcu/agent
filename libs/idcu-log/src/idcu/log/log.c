...
idcu_LogLevel idcu_log_get_level(void) { return log_level; }
...
void idcu_log_set_output(idcu_LogOutput output) { log_output = output; }
...
int idcu_log_rotate(void) { return rotate_log_files(); }
...
void idcu_log_printf(idcu_LogLevel level, const char *file, int line, const char *fmt, ...) {
    ...
    char header[1024];
    int header_len =
        snprintf(header, sizeof(header), "[%s] [%s] [%s:%d] ", time_buf, level_name, file, line);

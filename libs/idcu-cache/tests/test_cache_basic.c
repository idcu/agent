...
int main(void) {
    ...
    if (test_cache_init_destroy() != 0)
        failures++;
    if (test_cache_put_get_string() != 0)
        failures++;
    if (test_cache_put_get_int() != 0)
        failures++;
    if (test_cache_contains() != 0)
        failures++;
    if (test_cache_remove() != 0)
        failures++;
    if (test_cache_clear() != 0)
        failures++;
    if (test_cache_lru_eviction() != 0)
        failures++;

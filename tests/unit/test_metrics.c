...
static void test_metrics_register(void) {
    ...
    ret = idcu_metrics_register(&collector, "test_histogram", "A test histogram",
...
static void test_metrics_export_prometheus(void) {
    ...
    IDCU_TEST_ASSERT(strstr(buffer, "# HELP http_requests_total") != NULL,
                     "Should contain http_requests_total HELP");
    IDCU_TEST_ASSERT(strstr(buffer, "# TYPE http_requests_total counter") != NULL,
                     "Should contain http_requests_total TYPE");
    IDCU_TEST_ASSERT(strstr(buffer, "http_requests_total 100") != NULL,
                     "Should contain http_requests_total value 100");

    IDCU_TEST_ASSERT(strstr(buffer, "# HELP memory_usage_bytes") != NULL,
                     "Should contain memory_usage_bytes HELP");
    IDCU_TEST_ASSERT(strstr(buffer, "# TYPE memory_usage_bytes gauge") != NULL,
                     "Should contain memory_usage_bytes TYPE");
    IDCU_TEST_ASSERT(strstr(buffer, "memory_usage_bytes 1024000") != NULL,
                     "Should contain memory_usage_bytes value 1024000");

    IDCU_TEST_ASSERT(strstr(buffer, "# HELP request_duration_ms") != NULL,
                     "Should contain request_duration_ms HELP");
    IDCU_TEST_ASSERT(strstr(buffer, "# TYPE request_duration_ms summary") != NULL,
                     "Should contain request_duration_ms TYPE");
    IDCU_TEST_ASSERT(strstr(buffer, "request_duration_ms_sum 225") != NULL,
                     "Should contain request_duration_ms_sum 225");
    IDCU_TEST_ASSERT(strstr(buffer, "request_duration_ms_count 3") != NULL,
                     "Should contain request_duration_ms_count 3");
...
int main(void) {
    ...
    idcu_test_suite_add_test(&g_suite, "prometheus_exporter_init_destroy",
                             test_prometheus_exporter_init_destroy);
    idcu_test_suite_add_test(&g_suite, "global_metrics_init_destroy",
                             test_global_metrics_init_destroy);
    idcu_test_suite_add_test(&g_suite, "global_metrics_register_default",
                             test_global_metrics_register_default);
    idcu_test_suite_add_test(&g_suite, "coro_metrics_integration", test_coro_metrics_integration);
    idcu_test_suite_add_test(&g_suite, "msg_bus_metrics_integration",
                             test_msg_bus_metrics_integration);

...
static void test_health_overall_status(void) {
    ...
    IDCU_TEST_ASSERT(overall == IDCU_HEALTH_WARNING,
                     "Overall status should be WARNING after errors");

    idcu_health_reset_error_count(&monitor, 1);
    overall = idcu_health_get_overall_status(&monitor);
    IDCU_TEST_ASSERT(overall == IDCU_HEALTH_HEALTHY,
                     "Overall status should be HEALTHY after reset");
...
int main(void) {
    ...
    idcu_test_suite_add_test(&g_suite, "health_monitor_init_destroy",
                             test_health_monitor_init_destroy);
    idcu_test_suite_add_test(&g_suite, "health_register_unregister",
                             test_health_register_unregister);

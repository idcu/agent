...
static void test_config_init_shutdown(void) {
    ...
    IDCU_TEST_ASSERT(ret == IDCU_ERR_OK || ret == IDCU_ERR_CONFIG_LOAD,
                     "idcu_config_init should work");
...
static void test_config_has_section_key(void) {
    ...
    IDCU_TEST_ASSERT(idcu_config_has_section("nonexistent") == 0,
                     "Should not have nonexistent section");
    ...
    IDCU_TEST_ASSERT(idcu_config_has_key("test", "nonexistent") == 0,

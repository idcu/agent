...
static void test_enhanced_sandbox_init_null_param(void) {
    ...
    IDCU_TEST_ASSERT(ret == IDCU_ERR_INVALID_PARAM,
                     "idcu_enhanced_sandbox_init with NULL should fail");
    IDCU_TEST_PASS();
}
...
static void test_enhanced_sandbox_memory_regions(void) {
    ...
    int ret = idcu_enhanced_sandbox_add_memory_region(&sb, test_addr1, 1024,
...
static void test_enhanced_sandbox_memory_region_null_params(void) {
    ...
    int ret =
        idcu_enhanced_sandbox_add_memory_region(NULL, (void *)0x1000, 1024, IDCU_MEM_REGION_READ);
    ...
    ret =
        idcu_enhanced_sandbox_check_memory_access(NULL, (void *)0x1000, 1024, IDCU_MEM_REGION_READ);
    ...
    IDCU_TEST_ASSERT(ret == IDCU_ERR_INVALID_PARAM,
                     "check_memory_access with NULL addr should fail");
    ...
    IDCU_TEST_ASSERT(ret == IDCU_ERR_INVALID_PARAM,
                     "remove_memory_region with NULL sb should fail");

    ret = idcu_enhanced_sandbox_remove_memory_region(&sb, NULL);
    IDCU_TEST_ASSERT(ret == IDCU_ERR_INVALID_PARAM,
                     "remove_memory_region with NULL addr should fail");
...
static void test_enhanced_sandbox_memory_region_not_found(void) {
    ...
    IDCU_TEST_ASSERT(ret == IDCU_ERR_NOT_FOUND,
                     "remove_memory_region for non-existent region should fail");

    ret = idcu_enhanced_sandbox_check_memory_access(&sb, test_addr, 512, IDCU_MEM_REGION_READ);
    IDCU_TEST_ASSERT(ret == IDCU_ERR_PERM_DENIED,
                     "check_memory_access for non-existent region should fail");
...
static void test_enhanced_sandbox_memory_regions_full(void) {
    ...
    for (int i = 0; i < IDCU_MAX_MEMORY_REGIONS; i++) {
        int ret =
            idcu_enhanced_sandbox_add_memory_region(&sb, addrs[i], 1024, IDCU_MEM_REGION_READ);
        if (ret != IDCU_ERR_OK) {
            all_succeeded = 0;
            break;
        }
...
static void test_enhanced_sandbox_memory_region_partial_access(void) {
    ...
    ret = idcu_enhanced_sandbox_check_memory_access(&sb, (void *)((uintptr_t)test_addr - 1), 10,
                                                    IDCU_MEM_REGION_READ);
    IDCU_TEST_ASSERT(ret == IDCU_ERR_PERM_DENIED, "Access before region should fail");

    ret = idcu_enhanced_sandbox_check_memory_access(&sb, (void *)((uintptr_t)test_addr + 1000), 100,
                                                    IDCU_MEM_REGION_READ);
...
static void test_enhanced_sandbox_inactive(void) {
    ...
    IDCU_TEST_ASSERT(ret == IDCU_ERR_INVALID_PARAM,
                     "remove_memory_region on inactive sb should fail");

    ret = idcu_enhanced_sandbox_check_memory_access(&sb, test_addr, 512, IDCU_MEM_REGION_READ);
    IDCU_TEST_ASSERT(ret == IDCU_ERR_INVALID_PARAM,
                     "check_memory_access on inactive sb should fail");
...
static void test_enhanced_sandbox_memory_allocator(void) {
    ...
    IDCU_TEST_ASSERT(idcu_enhanced_sandbox_get_memory_usage(&sb) == 1024,
    ...
    IDCU_TEST_ASSERT(idcu_enhanced_sandbox_get_memory_usage(&sb) == 2024,
                     "Memory usage should be 2024");

    void *ptr3 = idcu_enhanced_sandbox_realloc(&sb, ptr1, 2048);
    IDCU_TEST_ASSERT(ptr3 != NULL, "realloc should succeed");
    IDCU_TEST_ASSERT(idcu_enhanced_sandbox_get_memory_usage(&sb) == 3048,
                     "Memory usage should be 3048");

    idcu_enhanced_sandbox_free(&sb, ptr2);
    IDCU_TEST_ASSERT(idcu_enhanced_sandbox_get_memory_usage(&sb) == 2048,
                     "Memory usage should be 2048 after free");

    idcu_enhanced_sandbox_free(&sb, ptr3);
    IDCU_TEST_ASSERT(idcu_enhanced_sandbox_get_memory_usage(&sb) == 0,
                     "Memory usage should be 0 after all freed");
...
static void test_enhanced_sandbox_access_macros(void) {
    ...
    idcu_enhanced_sandbox_add_memory_region(&sb, test_addr, 1024,
                                            IDCU_MEM_REGION_READ | IDCU_MEM_REGION_WRITE);
...
int main(void) {
    ...
    idcu_test_suite_add_test(&g_suite, "enhanced_sandbox_init_destroy",
                             test_enhanced_sandbox_init_destroy);
    idcu_test_suite_add_test(&g_suite, "enhanced_sandbox_init_null_param",
                             test_enhanced_sandbox_init_null_param);
    idcu_test_suite_add_test(&g_suite, "enhanced_sandbox_memory_regions",
                             test_enhanced_sandbox_memory_regions);
    idcu_test_suite_add_test(&g_suite, "enhanced_sandbox_memory_region_null_params",
                             test_enhanced_sandbox_memory_region_null_params);
    idcu_test_suite_add_test(&g_suite, "enhanced_sandbox_memory_region_not_found",
                             test_enhanced_sandbox_memory_region_not_found);
    idcu_test_suite_add_test(&g_suite, "enhanced_sandbox_memory_regions_full",
                             test_enhanced_sandbox_memory_regions_full);
    idcu_test_suite_add_test(&g_suite, "enhanced_sandbox_memory_region_partial_access",
                             test_enhanced_sandbox_memory_region_partial_access);
    idcu_test_suite_add_test(&g_suite, "enhanced_sandbox_syscall_whitelist",
                             test_enhanced_sandbox_syscall_whitelist);
    idcu_test_suite_add_test(&g_suite, "enhanced_sandbox_syscall_null_params",
                             test_enhanced_sandbox_syscall_null_params);
    idcu_test_suite_add_test(&g_suite, "enhanced_sandbox_syscall_not_found",
                             test_enhanced_sandbox_syscall_not_found);
    idcu_test_suite_add_test(&g_suite, "enhanced_sandbox_syscalls_full",
                             test_enhanced_sandbox_syscalls_full);
    idcu_test_suite_add_test(&g_suite, "enhanced_sandbox_resource_limits",
                             test_enhanced_sandbox_resource_limits);
    idcu_test_suite_add_test(&g_suite, "enhanced_sandbox_resource_limit_null_params",
                             test_enhanced_sandbox_resource_limit_null_params);
    idcu_test_suite_add_test(&g_suite, "enhanced_sandbox_cpu_limit_exceeded",
                             test_enhanced_sandbox_cpu_limit_exceeded);
    idcu_test_suite_add_test(&g_suite, "enhanced_sandbox_fd_limit_exceeded",
                             test_enhanced_sandbox_fd_limit_exceeded);
    idcu_test_suite_add_test(&g_suite, "enhanced_sandbox_zero_limits",
                             test_enhanced_sandbox_zero_limits);
    idcu_test_suite_add_test(&g_suite, "enhanced_sandbox_inactive", test_enhanced_sandbox_inactive);
    idcu_test_suite_add_test(&g_suite, "enhanced_sandbox_memory_allocator",
                             test_enhanced_sandbox_memory_allocator);
    idcu_test_suite_add_test(&g_suite, "enhanced_sandbox_memory_limit",
                             test_enhanced_sandbox_memory_limit);
    idcu_test_suite_add_test(&g_suite, "enhanced_sandbox_registry", test_enhanced_sandbox_registry);
    idcu_test_suite_add_test(&g_suite, "enhanced_sandbox_access_macros",
                             test_enhanced_sandbox_access_macros);

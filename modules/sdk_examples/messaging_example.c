...
static void message_handler(idcu_SdkContext* ctx, const idcu_SdkMessage* msg, void* user_data) {
    ...
    idcu_sdk_log_info(ctx, "Received message from module %u, priority %d", 
                      msg->source_id, msg->priority);
...
static int messaging_example_start(idcu_SdkContext* ctx) {
    ...
    idcu_sdk_broadcast_message(ctx, IDCU_MSG_PRIO_NORMAL, 
...
IDCU_SDK_MODULE_DEFINE(
    sdk_messaging_example,
    "1.0.0",
    "A messaging example module using IDCU SDK",
    messaging_example_init,
    messaging_example_start,
    messaging_example_stop,

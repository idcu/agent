...
idcu_metrics_integration_get_collector(idcu_MetricsIntegration *integration) {
    if (!integration || !integration->initialized) {
        return NULL;
    }
...
static int metrics_integration_module_run(void) { return 0; }
...
IDCU_REGISTER_MODULE(metrics_integration, IDCU_MODULE_VERSION(1, 0, 0),
                     metrics_integration_module_init, metrics_integration_module_run,

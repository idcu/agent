...
int idcu_basic_libs_init(idcu_BasicLibs* libs, const char* config_file, const char* log_file, idcu_LogLevel log_level)
{
    if (!libs) {
...
static int basic_libs_module_run(void)
{
    return 0;
}
...
IDCU_REGISTER_MODULE(basic_libs, IDCU_MODULE_VERSION(1, 0, 0), 
                      basic_libs_module_init, 
                      basic_libs_module_run, 
                      basic_libs_module_stop);

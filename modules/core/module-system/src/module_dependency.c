...
idcu_ModuleDependencyGraph *idcu_module_dependency_graph_create(void) {
    idcu_ModuleDependencyGraph *graph =
        (idcu_ModuleDependencyGraph *)malloc(sizeof(idcu_ModuleDependencyGraph));
    if (!graph) {
        return NULL;
    }
...
int idcu_module_dependency_graph_add_module(idcu_ModuleDependencyGraph *graph,
    ...
    if (dependencies && dependency_count > 0) {
        new_node->dependencies_copy =
            (idcu_ModuleDependency *)malloc(sizeof(idcu_ModuleDependency) * dependency_count);
...
static int topological_sort_visit(idcu_ModuleDependencyGraph *graph, int node_index,
...
int idcu_module_dependency_graph_resolve(idcu_ModuleDependencyGraph *graph, const char *module_name,
                                         idcu_ModuleDependencyNode **out_order, int *out_count) {
...
int idcu_module_dependency_graph_check_versions(idcu_ModuleDependencyGraph *graph) {
    ...
    for (int i = 0; i < graph->node_count; i++) {
        ...
        for (int j = 0; j < node->node.dependency_count; j++) {
            ...
            int ret = idcu_module_version_check_compat(current_version, &dep->required_version,
                                                       &dep->max_version, dep->compat_mode);

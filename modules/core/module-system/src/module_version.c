...
int idcu_version_format(const idcu_ModuleVersion *version, char *buffer, size_t buffer_size) {
    ...
    if (version->pre_release) {
        written = snprintf(buffer, buffer_size, "%u.%u.%u-%s", version->major, version->minor,
                           version->patch, version->pre_release);
    } else {
        written = snprintf(buffer, buffer_size, "%u.%u.%u", version->major, version->minor,
                           version->patch);
    }

    return (written >= 0 && (size_t)written < buffer_size) ? IDCU_SUCCESS
                                                           : IDCU_ERROR_BUFFER_TOO_SMALL;
}

idcu_VersionCompareResult idcu_version_compare(const idcu_ModuleVersion *v1,
                                               const idcu_ModuleVersion *v2) {
    if (!v1 || !v2) {
        return IDCU_VERSION_EQUAL;
    }

    if (v1->major < v2->major)
        return IDCU_VERSION_LESS;
    if (v1->major > v2->major)
        return IDCU_VERSION_GREATER;
...
int idcu_version_is_compatible(const idcu_ModuleVersion *current,
...
int idcu_version_check_dependency(const idcu_ModuleVersion *actual,
    ...
    if (dependency->has_max_version) {
        idcu_VersionCompareResult max_result =
            idcu_version_compare(actual, &dependency->max_version);
...
idcu_ModuleVersion idcu_version_create_pre(uint16_t major, uint16_t minor, uint16_t patch,

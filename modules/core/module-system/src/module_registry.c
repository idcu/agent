...
int idcu_module_registry_register(idcu_ModuleRegistry *registry, const idcu_ModuleInterface *iface,
    ...
    if (ret != IDCU_ERR_SUCCESS) {
        IDCU_LOG_ERROR("module_registry_register failed: lock error, code=%d (%s)", ret,
                       idcu_err_to_str(ret));
        return ret;
    }
    ...
    if (registry->count >= IDCU_MAX_REGISTERED_MODULES) {
        idcu_mutex_unlock(&registry->lock);
        IDCU_LOG_ERROR("module_registry_register failed: module registry full (max=%d)",
                       IDCU_MAX_REGISTERED_MODULES);
...
int idcu_module_registry_init_module(idcu_ModuleRegistry *registry, uint32_t module_id) {
    ...
    int ret = idcu_mutex_lock(&registry->lock);
    if (ret != IDCU_ERR_SUCCESS) {
        IDCU_LOG_ERROR("module_registry_init_module failed: lock error, code=%d (%s)", ret,
                       idcu_err_to_str(ret));
        return ret;
    }
    ...
    if (mod->iface->init) {
        ...
        if (result != IDCU_ERR_SUCCESS) {
            ...
            idcu_mutex_unlock(&registry->lock);
            IDCU_LOG_ERROR("module %s init failed, code=%d (%s)", mod->iface->name, result,
...
int idcu_module_registry_run_module(idcu_ModuleRegistry *registry, uint32_t module_id) {
    ...
    int ret = idcu_mutex_lock(&registry->lock);
    if (ret != IDCU_ERR_SUCCESS) {
        IDCU_LOG_ERROR("module_registry_run_module failed: lock error, code=%d (%s)", ret,
                       idcu_err_to_str(ret));
        return ret;
    }
    ...
    if (mod->state == IDCU_MOD_STATE_UNINIT) {
        ...
        if (ret != IDCU_ERR_SUCCESS) {
            IDCU_LOG_ERROR("module_registry_run_module failed: lock error after init, code=%d (%s)",
    ...
    if (mod->iface->run) {
        ...
        if (result != IDCU_ERR_SUCCESS) {
            mod->state = IDCU_MOD_STATE_ERROR;
            idcu_mutex_unlock(&registry->lock);
            IDCU_LOG_ERROR("module %s run failed, code=%d (%s)", mod->iface->name, result,
                           idcu_err_to_str(result));
            return result;
        }
...
int idcu_module_registry_discover_modules(idcu_ModuleRegistry *registry) {
    ...
    ret = idcu_module_registry_register(registry, &__idcu_module_collect_module,
    ...
    ret = idcu_module_registry_register(registry, &__idcu_module_heartbeat_module,
                                        IDCU_MOD_PRIO_NORMAL);
    if (ret != IDCU_ERR_SUCCESS && ret != IDCU_ERR_ALREADY_EXISTS) {
        return ret;
    }
    ret = idcu_module_registry_register(registry, &__idcu_module_healthcheck_module,
                                        IDCU_MOD_PRIO_NORMAL);
    if (ret != IDCU_ERR_SUCCESS && ret != IDCU_ERR_ALREADY_EXISTS) {
        return ret;
    }
    ret = idcu_module_registry_register(registry, &__idcu_module_metrics_module,
                                        IDCU_MOD_PRIO_NORMAL);
    if (ret != IDCU_ERR_SUCCESS && ret != IDCU_ERR_ALREADY_EXISTS) {
        return ret;
    }
    ret =
        idcu_module_registry_register(registry, &__idcu_module_alert_module, IDCU_MOD_PRIO_NORMAL);
    if (ret != IDCU_ERR_SUCCESS && ret != IDCU_ERR_ALREADY_EXISTS) {
        return ret;
    }
#endif
...
int idcu_module_registry_build_dependency_graph(idcu_ModuleRegistry *registry) {
    ...
    int ret = idcu_mutex_lock(&registry->lock);
    if (ret != IDCU_ERR_SUCCESS) {
        IDCU_LOG_ERROR("module_registry_build_dependency_graph failed: lock error, code=%d (%s)",
                       ret, idcu_err_to_str(ret));
        return ret;
    }
    ...
    for (uint32_t i = 0; i < registry->count; i++) {
        ...
        for (int d = 0; d < mod->iface->dependency_count; d++) {
            ...
            if (!dep_found) {
                idcu_mutex_unlock(&registry->lock);
                IDCU_LOG_ERROR("module_registry_build_dependency_graph failed: dependency %s not "
                               "found for module %s",
                               dep_name, mod->iface->name);
                return IDCU_ERR_NOT_FOUND;
            }
        }
    }
...
int idcu_module_registry_topological_sort(idcu_ModuleRegistry *registry) {
    ...
    int ret = idcu_mutex_lock(&registry->lock);
    if (ret != IDCU_ERR_SUCCESS) {
        IDCU_LOG_ERROR("module_registry_topological_sort failed: lock error, code=%d (%s)", ret,
                       idcu_err_to_str(ret));
        return ret;
    }
...
int idcu_module_registry_check_dependency_versions(idcu_ModuleRegistry *registry) {
    ...
    int ret = idcu_mutex_lock(&registry->lock);
    if (ret != IDCU_ERR_SUCCESS) {
        IDCU_LOG_ERROR("module_registry_check_dependency_versions failed: lock error, code=%d (%s)",
                       ret, idcu_err_to_str(ret));
        return ret;
    }
        ...
        for (int d = 0; d < mod->iface->dependency_count; d++) {
            ...
            for (uint32_t j = 0; j < registry->count; j++) {
                if (strcmp(registry->modules[j].iface->name, dep->dependency_name) == 0) {
                    ...
                    int version_check =
                        idcu_version_check_dependency(&dep_mod->iface->version, dep);
                    if (version_check != IDCU_SUCCESS) {
                        ...
                        idcu_mutex_unlock(&registry->lock);
                        IDCU_LOG_ERROR("version check failed for %s -> %s: actual=%s, required "
                                       "min=%s, error=%s",
                    ...
                    dep_found = 1;
                    IDCU_LOG_DEBUG("version check passed: %s -> %s", mod->iface->name,
                                   dep->dependency_name);
                    break;
                }
            }
            ...
            if (!dep_found) {
                idcu_mutex_unlock(&registry->lock);
                IDCU_LOG_ERROR("module_registry_check_dependency_versions failed: dependency %s "
                               "not found for module %s",
...
int idcu_module_registry_get_module_version(idcu_ModuleRegistry *registry, const char *module_name,
    ...
    *out_module = NULL;
    ...
    for (uint32_t i = 0; i < registry->count; i++) {
        if (strcmp(registry->modules[i].iface->name, module_name) == 0) {
            ...
            if (compatible) {
                if (!best_match || idcu_version_compare(ver, &best_match->iface->version) ==
                                       IDCU_VERSION_GREATER) {
...
int idcu_module_registry_apply_config(idcu_ModuleRegistry *registry) {
    ...
    for (uint32_t i = 0; i < registry->count; i++) {
        ...
        int enabled =
            idcu_module_category_is_module_enabled(&registry->category_mgr, mod->iface->name);
        mod->enabled = enabled;

        idcu_ModulePrio prio =
            idcu_module_category_get_module_priority(&registry->category_mgr, mod->iface->name);
        mod->priority = prio;

                                      ...
                                      const char* config_file)
{
    ...
    const char* known_modules[] = {
        ...
        "heartbeat_module", "healthcheck_module", "metrics_module", "alert_module",
        NULL
    };
    
    for (int i = 0; known_modules[i] != NULL && mgr->module_config_count < IDCU_MAX_REGISTERED_MODULES; i++) {
        ...
        IDCU_LOG_INFO("loaded module config: %s (category: %s, enabled: %d)",
                     module_name, mod_cfg->category_name, mod_cfg->enabled);
    }

...
#define IDCU_MODULE_DESCRIPTION_MAX 256
    ...
    typedef void (*idcu_ModuleEventHandler)(uint32_t module_id, idcu_ModuleEvent event,
                                            void* user_data);
    ...
    int idcu_module_register(idcu_ModuleManager* manager, const char* name, const char* version,
                             const char* description, idcu_ModuleInterface* iface,
                             idcu_ModulePrio priority);
    int idcu_module_register_with_deps(idcu_ModuleManager* manager, const char* name,
                                       const char* version, const char* description,
                                       idcu_ModuleInterface* iface, idcu_ModulePrio priority,
                                       const idcu_ModuleDependency* dependencies, int dep_count);
    ...
    int idcu_module_add_dependency(idcu_ModuleManager* manager, uint32_t module_id,
                                   const char* dep_name, const char* required_version);
    ...
    int idcu_module_set_event_handler(idcu_ModuleManager* manager, uint32_t module_id,
                                      idcu_ModuleEventHandler handler, void* user_data);

    idcu_ModuleState idcu_module_get_state(idcu_ModuleManager* manager, uint32_t module_id);
    int  idcu_module_get_error(idcu_ModuleManager* manager, uint32_t module_id, char* buffer,
                               size_t buffer_size);
    void idcu_module_clear_error(idcu_ModuleManager* manager, uint32_t module_id);

    int idcu_module_version_parse(const char* version_str, uint32_t* major, uint32_t* minor,
                                  uint32_t* patch);

...
typedef struct
{
    uint16_t    major;        // 主版本号 (不兼容修�?
    uint16_t    minor;        // 次版本号 (向后兼容新增功能)
    uint16_t    patch;        // 修订�?(向后兼容 bug 修复)
    const char* pre_release;  // 预发布版本标�?(�?"alpha", "beta.1")
} idcu_ModuleVersion;
...
typedef struct
{
    ...
    idcu_ModuleVersion min_version;      // 最低要求版�?
    idcu_ModuleVersion max_version;      // 最高兼容版�?(可�?
    int                has_max_version;  // 是否设置了最高版�?
...
typedef struct
{
    ...
    const idcu_ModuleDependency* dependencies;      // 依赖的模块列�?
...
#define IDCU_REGISTER_MODULE(name, ver, init_fn, run_fn, stop_fn)                                  \
    const idcu_ModuleInterface __idcu_module_##name __attribute__((section(".modules"), used)) = { \
...
#define IDCU_REGISTER_MODULE(name, ver, init_fn, run_fn, stop_fn)                  \
    __pragma(section(".modules", read)) __declspec(allocate(".modules"))           \
    const idcu_ModuleInterface __idcu_module_##name = {#name,   ver,    NULL,   0, \
                                                       init_fn, run_fn, stop_fn}
#define IDCU_REGISTER_MODULE_WITH_DEPS(name, ver, deps, dep_count, init_fn, run_fn, stop_fn) \
    __pragma(section(".modules", read)) __declspec(allocate(".modules"))                     \
    const idcu_ModuleInterface __idcu_module_##name = {#name,   ver,    deps,   dep_count,   \
                                                       init_fn, run_fn, stop_fn}
#else
#define IDCU_REGISTER_MODULE(name, ver, init_fn, run_fn, stop_fn)                  \
    const idcu_ModuleInterface __idcu_module_##name = {#name,   ver,    NULL,   0, \

...
static int64_t get_current_timestamp(void) { return (int64_t)time(NULL) * 1000; }
...
int idcu_module_comm_send_request(idcu_ModuleCommContext *ctx, const char *target_module,
                                  ...
                                  idcu_ModuleResponseCallback callback, void *callback_user_data,
...
int idcu_module_comm_send_response(idcu_ModuleCommContext *ctx, const idcu_ModuleMessage *request,
                                   int error_code, const uint8_t *payload, uint32_t payload_size) {
    if (!ctx || !ctx->msg_bus || !request)
        return IDCU_ERR_INVALID_PARAM;

    idcu_StackContext stack_ctx = {0};
    return idcu_msg_send(ctx->msg_bus, ctx->module_id, request->source_module_id, request->priority,
                         &stack_ctx);
}

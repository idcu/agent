...
idcu_ModuleLifecycleContext* idcu_module_lifecycle_create_context(void) {
    idcu_ModuleLifecycleContext* ctx = (idcu_ModuleLifecycleContext*)malloc(sizeof(idcu_ModuleLifecycleContext));
    if (!ctx) {
        return NULL;
    }
...
void idcu_module_lifecycle_set_config(idcu_ModuleLifecycleContext* ctx, const idcu_ModuleLifecycleConfig* config) {
...
idcu_ModuleState idcu_module_lifecycle_get_state(idcu_ModuleLifecycleContext* ctx) {
    if (!ctx) return IDCU_MOD_STATE_ERROR;
    return ctx->state;
}

...
idcu_ModuleErrorContext* idcu_module_error_create_context(void) {
    idcu_ModuleErrorContext* ctx = (idcu_ModuleErrorContext*)malloc(sizeof(idcu_ModuleErrorContext));
    if (!ctx) {
        return NULL;
    }
...
int idcu_module_error_raise(idcu_ModuleErrorContext* ctx,
                            ...
                            const char* file,
                            int line) {
    return idcu_module_error_raise_with_data(ctx, error_code, error_message, module_name, file, line, NULL);
}
...
int idcu_module_error_get_last(idcu_ModuleErrorContext* ctx, idcu_ModuleError* out_error) {
    if (!ctx || !out_error) return IDCU_ERR_INVALID_PARAM;

...
#define IDCU_SDK_VERSION "1.0.0"
    ...
    typedef void (*idcu_SdkMessageHandler)(idcu_SdkContext* ctx, const idcu_SdkMessage* msg,
                                           void* user_data);
    ...
    void idcu_sdk_set_module_info(idcu_SdkContext* ctx, uint32_t module_id,
    ...
    int idcu_sdk_register_message_handler(idcu_SdkContext* ctx, idcu_SdkMessageHandler handler,
                                          void* user_data);

    int idcu_sdk_log(idcu_SdkContext* ctx, idcu_LogLevel level, const char* fmt, ...);

#define idcu_sdk_log_debug(ctx, fmt, ...) idcu_sdk_log(ctx, IDCU_LOG_DEBUG, fmt, ##__VA_ARGS__)

#define idcu_sdk_log_info(ctx, fmt, ...) idcu_sdk_log(ctx, IDCU_LOG_INFO, fmt, ##__VA_ARGS__)

#define idcu_sdk_log_warn(ctx, fmt, ...) idcu_sdk_log(ctx, IDCU_LOG_WARN, fmt, ##__VA_ARGS__)

#define idcu_sdk_log_error(ctx, fmt, ...) idcu_sdk_log(ctx, IDCU_LOG_ERROR, fmt, ##__VA_ARGS__)

#define idcu_sdk_log_fatal(ctx, fmt, ...) idcu_sdk_log(ctx, IDCU_LOG_FATAL, fmt, ##__VA_ARGS__)

#define IDCU_SDK_MODULE_DEFINE(module_name, module_version, module_desc, init_fn, start_fn, \
                               stop_fn, destroy_fn)                                         \
    ...
    {                                                                                       \
        ...
        if (init_fn)                                                                        \
            return init_fn(g_sdk_ctx);                                                      \
        return IDCU_ERR_OK;                                                                 \
    }                                                                                       \
                                                                                            \
    static int sdk_wrapper_start(void)                                                      \
    {                                                                                       \
        if (start_fn && g_sdk_ctx)                                                          \
            return start_fn(g_sdk_ctx);                                                     \
        return IDCU_ERR_OK;                                                                 \
    }                                                                                       \
                                                                                            \
    static int sdk_wrapper_stop(void)                                                       \
    {                                                                                       \
        if (stop_fn && g_sdk_ctx)                                                           \
            return stop_fn(g_sdk_ctx);                                                      \
        return IDCU_ERR_OK;                                                                 \
    }                                                                                       \
                                                                                            \
    static void sdk_wrapper_destroy(void)                                                   \
    {                                                                                       \
        if (destroy_fn && g_sdk_ctx)                                                        \
            destroy_fn(g_sdk_ctx);                                                          \
        if (g_sdk_ctx)                                                                      \
        {                                                                                   \
            ...
            g_sdk_ctx = NULL;                                                               \
                                                                                            ...
                                                                                            \
    IDCU_REGISTER_MODULE(module_name, IDCU_MODULE_VERSION(1, 0, 0), sdk_wrapper_init,       \
                         sdk_wrapper_start, sdk_wrapper_stop)

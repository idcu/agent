...
int idcu_plugin_version_parse(const char *version_str, uint32_t *major, uint32_t *minor,
                              uint32_t *patch) {
...
int idcu_plugin_version_compare(uint32_t maj1, uint32_t min1, uint32_t pat1, uint32_t maj2,
                                uint32_t min2, uint32_t pat2) {
    if (maj1 != maj2) {
...
int idcu_plugin_ecosystem_add_plugin(idcu_PluginEcosystem *eco, const char *name,
                                     const char *version, const char *description) {
    if (!eco || !name || !version)
        return IDCU_ERR_INVALID_PARAM;
    if (eco->plugin_count >= IDCU_MAX_PLUGINS)
        return IDCU_ERR_NO_MEMORY;
    if (idcu_plugin_ecosystem_find_plugin(eco, name))
...
int idcu_plugin_ecosystem_set_dependencies(idcu_PluginEcosystem *eco, const char *name,
                                           const idcu_PluginDependency *deps, int count) {
    if (!eco || !name || !deps || count < 0 || count > IDCU_MAX_DEPENDENCIES)
        return IDCU_ERR_INVALID_PARAM;
...
int idcu_plugin_ecosystem_check_compatibility(idcu_PluginEcosystem *eco, const char *name,
                                              const char *required_version) {
    if (!eco || !name || !required_version)
        return IDCU_ERR_INVALID_PARAM;
    ...
    if (idcu_plugin_version_parse(required_version, &req_major, &req_minor, &req_patch) !=
        IDCU_ERR_OK) {
        return IDCU_ERR_INVALID_PARAM;
    }

    int cmp = idcu_plugin_version_compare(info->major, info->minor, info->patch, req_major,
                                          req_minor, req_patch);
    if (cmp < 0) {

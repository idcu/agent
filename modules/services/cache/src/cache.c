...
int idcu_cache_put(idcu_Cache *cache, const char *key, const void *value, size_t size,
                   uint64_t ttl_ms) {
    ...
    IDCU_LOG_DEBUG("[cache] Put key: %s, size: %zu, ttl: %llu", key, size,
...
int idcu_cache_get_string(idcu_Cache *cache, const char *key, char *buffer, size_t buffer_size) {
    if (!cache || !key || !buffer || buffer_size == 0)
        return IDCU_ERR_INVALID_PARAM;

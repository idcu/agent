...
int idcu_enhanced_sandbox_add_memory_region(idcu_EnhancedSandbox *sb, void *addr, size_t size,
                                            uint32_t perm) {
    if (!sb || !sb->active)
        return IDCU_ERR_INVALID_PARAM;
    if (sb->mem_region_count >= IDCU_MAX_MEMORY_REGIONS)
...
int idcu_enhanced_sandbox_check_memory_access(idcu_EnhancedSandbox *sb, void *addr, size_t size,
...
int idcu_enhanced_sandbox_add_syscall(idcu_EnhancedSandbox *sb, int syscall_num, const char *name) {
    if (!sb || !sb->active)
        return IDCU_ERR_INVALID_PARAM;
    if (sb->syscall_count >= IDCU_MAX_SYSCALLS)
        return IDCU_ERR_NO_MEMORY;
    if (!name)
        return IDCU_ERR_INVALID_PARAM;

    sb->syscalls[sb->syscall_count].syscall_num = syscall_num;
    strncpy(sb->syscalls[sb->syscall_count].name, name,
            sizeof(sb->syscalls[sb->syscall_count].name) - 1);
...
int idcu_enhanced_sandbox_check_cpu_usage(idcu_EnhancedSandbox *sb) {
    if (!sb || !sb->active)
        return IDCU_ERR_INVALID_PARAM;
    if (sb->max_cpu_time_ms == 0)
        return IDCU_ERR_OK;
    if (sb->used_cpu_time_ms >= sb->max_cpu_time_ms)
        return IDCU_ERR_PERM_DENIED;
    return IDCU_ERR_OK;
}
...
int idcu_enhanced_sandbox_check_fd_usage(idcu_EnhancedSandbox *sb, uint32_t requested_fds) {
    if (!sb || !sb->active)
        return IDCU_ERR_INVALID_PARAM;
    if (sb->max_file_descriptors == 0)
        return IDCU_ERR_OK;
    if (sb->used_file_descriptors + requested_fds > sb->max_file_descriptors)
        return IDCU_ERR_PERM_DENIED;
    return IDCU_ERR_OK;
}
...
void *idcu_enhanced_sandbox_malloc(idcu_EnhancedSandbox *sb, size_t size) {
    ...
    if (sb->max_memory_bytes > 0 && sb->used_memory_bytes + size > sb->max_memory_bytes) {
        return NULL;
    }
    ...
    int ret = idcu_enhanced_sandbox_add_memory_region(
        sb, ptr, size, IDCU_MEM_REGION_READ | IDCU_MEM_REGION_WRITE | IDCU_MEM_REGION_ALLOCATED);
    if (ret != IDCU_ERR_OK) {
        free(ptr);
        return NULL;
    }
...
void *idcu_enhanced_sandbox_calloc(idcu_EnhancedSandbox *sb, size_t nmemb, size_t size) {
    ...
    if (sb->max_memory_bytes > 0 && sb->used_memory_bytes + total_size > sb->max_memory_bytes) {
        return NULL;
    }
...
void *idcu_enhanced_sandbox_realloc(idcu_EnhancedSandbox *sb, void *ptr, size_t new_size) {
    if (!sb || !sb->active)
        return NULL;
    if (ptr == NULL)
        return idcu_enhanced_sandbox_malloc(sb, new_size);
    if (new_size == 0) {
...
int idcu_enhanced_sandbox_registry_check_memory_access(uint32_t module_id, void *addr, size_t size,
                                                       uint32_t required_perm) {

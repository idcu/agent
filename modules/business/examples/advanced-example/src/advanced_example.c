...
{
    (void)user_data;
    IDCU_LOG_INFO("[ALERT CALLBACK] %s: %s", 
                  event->alert_name, event->message);
}
...
static int advanced_example_init(void)
{
    ...
    ret = idcu_metrics_register(&g_metrics, "request_count", "Total HTTP requests", IDCU_METRIC_COUNTER);
    idcu_metrics_register(&g_metrics, "response_time_ms", "Response time", IDCU_METRIC_HISTOGRAM);
    idcu_metrics_register(&g_metrics, "active_connections", "Active connections", IDCU_METRIC_GAUGE);

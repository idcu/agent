...
static int cache_module_run() { return 0; }
...
int idcu_cache_module_destroy_cache(int cache_id) {
    ...
    if (cache_id < 0 || cache_id >= IDCU_CACHE_MAX_MODULE_CACHES ||
        !g_cache_module->caches[cache_id]) {
        return -1;
    }
...
int idcu_cache_module_put(int cache_id, const char *key, const void *value, size_t size,
                          uint64_t ttl_ms) {
    if (!g_cache_module || !g_cache_module->initialized) {
        return -1;
    }
    ...
    return idcu_cache_put(g_cache_module->caches[cache_id], key, value, size, ttl_ms) == IDCU_ERR_OK
               ? 0
               : -1;
}
...
int idcu_cache_module_get(int cache_id, const char *key, void *buffer, size_t *buffer_size) {
    ...
    return idcu_cache_get(g_cache_module->caches[cache_id], key, buffer, buffer_size) == IDCU_ERR_OK
               ? 0
               : -1;
}
...
int idcu_cache_module_put_string(int cache_id, const char *key, const char *value,
                                 uint64_t ttl_ms) {
    if (!g_cache_module || !g_cache_module->initialized) {
        return -1;
    }
    ...
    return idcu_cache_put_string(g_cache_module->caches[cache_id], key, value, ttl_ms) ==
                   IDCU_ERR_OK
               ? 0
               : -1;
}
...
int idcu_cache_module_get_string(int cache_id, const char *key, char *buffer, size_t buffer_size) {
    ...
    return idcu_cache_get_string(g_cache_module->caches[cache_id], key, buffer, buffer_size) ==
                   IDCU_ERR_OK
               ? 0
               : -1;
}
...
int idcu_cache_module_put_int(int cache_id, const char *key, int64_t value, uint64_t ttl_ms) {
    ...
    return idcu_cache_put_int(g_cache_module->caches[cache_id], key, value, ttl_ms) == IDCU_ERR_OK
               ? 0
               : -1;
}
...
IDCU_REGISTER_MODULE(cache_module, IDCU_MODULE_VERSION(1, 0, 0), cache_module_init,
                     cache_module_run, cache_module_stop);
